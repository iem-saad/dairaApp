...
import com.example.dairaapp.AdminPanel.EventsMainActivity;
...
public class AssignOCSubeventMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_assign_ocsubevent_main);
        events_spinner = findViewById(R.id.subeventsspinner);
        mentor_spinner = findViewById(R.id.submentorsspinner);
    ...
    public void load_mentor_data(){
        FirebaseDatabase.getInstance().getReference().child("ocs").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot data : snapshot.getChildren()){
    ...
    public void assign_subevent(View view) {
        ...
        map.put("oc_name", mentor_list.get(mentor_spinner.getSelectedItemPosition()));
        map.put("oc_mail", selected_mentor);
        map.put("sb_name", event_list.get(events_spinner.getSelectedItemPosition()));
        map.put("sb_id", selected_event);
        FirebaseDatabase.getInstance().getReference().child("subeventoc").push().setValue(map).
                addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void unused) {
                        Intent intent = new Intent(AssignOCSubeventMainActivity.this, ShowSEOCMainActivity.class);
                        startActivity(intent);
                        Toast.makeText(AssignOCSubeventMainActivity.this, "Added Assignment!", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(AssignOCSubeventMainActivity.this, "Error in Adding Assignment!", Toast.LENGTH_SHORT).show();
            }
        });
    }
}